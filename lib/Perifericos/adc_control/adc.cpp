...
namespace ADC
{
    ...
    int AdcController::GetRaw()
    {
        ...
        this->init_config.clk_src = ADC_RTC_CLK_SRC_DEFAULT;
    ...
    {
        esp_err_t status = adc_cali_scheme_line_fitting_check_efuse(&this->cali_val);
        ...
        {
            ESP_LOGE("ADC", "ESP_ERR_INVALID_STATE");
        }
        else if (status == ESP_ERR_NOT_SUPPORTED)
        {
            ESP_LOGE("ADC", "ESP_ERR_NOT_SUPPORTED");
        }
